...
function getPaymentConfig_() { const properties=PropertiesService.getScriptProperties(); return { mode:paymentMode_(properties), bankCode:properties.getProperty('PAYMENT_BANK_CODE')||'', bankName:properties.getProperty('PAYMENT_BANK_NAME')||'', accountName:properties.getProperty('PAYMENT_ACCOUNT_NAME')||'', accountNo:properties.getProperty('PAYMENT_ACCOUNT_NO')||'', publicSiteUrl:properties.getProperty('PUBLIC_SITE_URL')||'', webhookConfigured:!!properties.getProperty('PAYMENT_WEBHOOK_SECRET') }; }
...
function bookingHealth_() { const ss=getBookingSpreadsheet_(), properties=PropertiesService.getScriptProperties(), mode='manual'; const checks=Object.keys(BOOKING_SHEETS).map(name=>({name:name,ok:!!ss.getSheetByName(name)})); const paymentConfigured=['PAYMENT_BANK_CODE','PAYMENT_BANK_NAME','PAYMENT_ACCOUNT_NAME','PAYMENT_ACCOUNT_NO'].every(key=>!!properties.getProperty(key)); return {service:'booking-payment',ok:checks.every(c=>c.ok)&&paymentConfigured,checks:checks,emailConfigured:!!(properties.getProperty('OWNER_EMAIL')||Session.getEffectiveUser().getEmail()),paymentConfigured:paymentConfigured,paymentMode:mode,sepayDeferred:true,timestamp:new Date().toISOString()}; }
