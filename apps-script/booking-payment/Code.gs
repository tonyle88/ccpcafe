...
const DEFAULT_PRICING_POLICY = Object.freeze({twoPeoplePercent:10,threePeoplePercent:15,twoPeopleText:'Đi 2 người được giảm {percent}% — bạn tiết kiệm {discount}, còn {final}.',threePeopleText:'Đi 3 người được giảm {percent}% — bạn tiết kiệm {discount}, còn {final}.'});
...
function doGet(e) {
  try {
    ...
    if(action==='getPaymentConfig') return bookingJson_(true,getPaymentConfig_());
...
function setupBookingSpreadsheet() { const ss=getBookingSpreadsheet_(); ss.setSpreadsheetTimeZone('Asia/Ho_Chi_Minh'); Object.keys(BOOKING_SHEETS).forEach(name=>bookingEnsureSheet_(ss,name,BOOKING_SHEETS[name])); bookingFormatSheet_(ss.getSheetByName('Bookings')); return bookingHealth_(); }
...
function register_(data) {
  ...
  try {
    ...
    const pricing=pricingPolicy_(),discountPercent=clean.partySize===2?pricing.twoPeoplePercent:(clean.partySize>=3?pricing.threePeoplePercent:0),discountRate=discountPercent/100;
    ...
    const record={
      ...
      'Preferred Date':bookingDateValue_(clean.preferredDate),'Party Size':clean.partySize,'Topic':clean.topic,'Order ID':orderId,'Base Amount':pkg.price,'Discount Amount':discount,
    ...
    return { orderId:orderId, baseAmount:pkg.price, discountAmount:discount, discountPercent:discountPercent, amount:finalAmount, currency:'VND', status:'PENDING_PAYMENT', paymentUrl:'payment.html?orderId='+encodeURIComponent(orderId) };
...
function pricingPolicy_() {
  const cache=CacheService.getScriptCache(),cached=cache.get('pricing-policy-v1'); if(cached){try{return JSON.parse(cached);}catch(_) {}}
  const endpoint=String(PropertiesService.getScriptProperties().getProperty('CONTENT_WEB_APP_URL')||'').trim();
  if(!/^https:\/\/(script\.google\.com|script\.googleusercontent\.com)\//i.test(endpoint)) return DEFAULT_PRICING_POLICY;
  try {
    const separator=endpoint.indexOf('?')>=0?'&':'?',response=UrlFetchApp.fetch(endpoint+separator+'action=publicInit',{muteHttpExceptions:true,followRedirects:true});
    if(response.getResponseCode()!==200)return DEFAULT_PRICING_POLICY;
    const payload=JSON.parse(response.getContentText()),source=payload&&payload.ok&&payload.data&&payload.data.config&&payload.data.config.pricing;
    if(!source)return DEFAULT_PRICING_POLICY;
    const two=Number(source.twoPeoplePercent),three=Number(source.threePeoplePercent),policy={twoPeoplePercent:Number.isFinite(two)&&two>=0&&two<=90?two:10,threePeoplePercent:Number.isFinite(three)&&three>=0&&three<=90?three:15,twoPeopleText:String(source.twoPeopleText||DEFAULT_PRICING_POLICY.twoPeopleText).slice(0,180),threePeopleText:String(source.threePeopleText||DEFAULT_PRICING_POLICY.threePeopleText).slice(0,180)};
    if(policy.threePeoplePercent<policy.twoPeoplePercent)return DEFAULT_PRICING_POLICY;
    cache.put('pricing-policy-v1',JSON.stringify(policy),300); return policy;
  } catch(_) { return DEFAULT_PRICING_POLICY; }
}
...
function paymentMode_(_) { return 'manual'; }
function paymentBankCode_(properties) { const raw=String(properties.getProperty('PAYMENT_BANK_CODE')||'').trim(),name=String(properties.getProperty('PAYMENT_BANK_NAME')||'').toUpperCase().replace(/[^A-Z0-9]/g,''),aliases={VIETCOMBANK:'VCB',NGANHANGVIETCOMBANK:'VCB',TECHCOMBANK:'TCB',VIETINBANK:'ICB',AGRIBANK:'VBA',MBBANK:'MB',MILITARYBANK:'MB',SACOMBANK:'STB',ACB:'ACB',BIDV:'BIDV',VPBANK:'VPB',TPBANK:'TPB',VIB:'VIB',SHB:'SHB',OCB:'OCB',HDBANK:'HDB',SEABANK:'SEAB',EXIMBANK:'EIB',NAMABANK:'NAB',PVCOMBANK:'PVCB'}; return /^[A-Za-z0-9]{2,20}$/.test(raw)?raw:(aliases[name]||''); }
function getPaymentConfig_() { const properties=PropertiesService.getScriptProperties(); return { mode:paymentMode_(properties), bankCode:paymentBankCode_(properties), bankName:properties.getProperty('PAYMENT_BANK_NAME')||'', accountName:properties.getProperty('PAYMENT_ACCOUNT_NAME')||'', accountNo:properties.getProperty('PAYMENT_ACCOUNT_NO')||'', publicSiteUrl:properties.getProperty('PUBLIC_SITE_URL')||'', webhookConfigured:!!properties.getProperty('PAYMENT_WEBHOOK_SECRET') }; }
...
function bookingCustomerEmailHtml_(clean,pkg,orderId,amount,paymentUrl) {
  const name=bookingHtml_(clean.name),packageName=bookingHtml_(pkg.name),safeOrder=bookingHtml_(orderId),date=bookingHtml_(bookingDateVi_(clean.preferredDate)),amountText=bookingHtml_(amount.toLocaleString('vi-VN')+' ₫');
...
function bookingFormatSheet_(sheet){if(!sheet||sheet.getLastRow()<2)return;const headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0].map(String),rows=sheet.getLastRow()-1;['Created At','Paid At','Last Updated At'].forEach(header=>{const index=headers.indexOf(header);if(index>=0)sheet.getRange(2,index+1,rows,1).setNumberFormat('dd/MM/yyyy HH:mm:ss');});const preferredIndex=headers.indexOf('Preferred Date');if(preferredIndex>=0){const range=sheet.getRange(2,preferredIndex+1,rows,1),values=range.getValues().map(row=>{const value=row[0];return [/^\d{4}-\d{2}-\d{2}$/.test(String(value))?bookingDateValue_(String(value)):value];});range.setValues(values).setNumberFormat('dd/MM/yyyy');}}
